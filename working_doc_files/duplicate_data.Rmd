...
is_duplicate <- function(data) {
  ...
  for (i in 1:(nrow(data) - 1)) {
    for (j in (i + 1):nrow(data)) {
      ...
      # If there are at least 7 matches, mark the second row as a duplicate
      if (num_matches >= 8) {
        dup[j] <- TRUE
      }
    }
...
test <- manual %>% 
  ...
  select(c_system_abbr, all_of(relevant_cols), n_dups) %>% 
  arrange(c_ind_first, c_ind_last) %>% 
  group_by(c_system_abbr) %>% 
  group_split() %>% 
  lapply(is_duplicate) 
test

##PROBELMS WITH: BOP (RUBEN GIL, SEAN MCQUIDDY, THOMAS CHURCH, VICTOR BIGMAN), FL (JESSIE SINGLETON), IA (TIMOTHY BRYANT) 
```
