...
#Total present data percentage by system 
```{r}
...
dcra_table_variable_bar_cleaned <- dcra_table_variable %>% 
  mutate(c_ind_full_name = 
           case_when(
             c_ind_full_name > threshold ~ 1, 
             TRUE ~ 0),
         c_ind_dob_year = 
           case_when(
             c_ind_dob_year > threshold ~ 1, 
             TRUE ~ 0),
         c_ind_gender = 
           case_when(
             c_ind_gender > threshold ~ 1, 
             TRUE ~ 0),
         c_ind_race = 
           case_when(
             c_ind_race > threshold ~ 1, 
             TRUE ~ 0),
         c_ind_ethnicity = 
           case_when(
             c_ind_ethnicity > threshold ~ 1, 
             TRUE ~ 0),
         c_ind_dod_ymd = 
           case_when(
             c_ind_dod_ymd > threshold ~ 1, 
             TRUE ~ 0),
         ind_tod = 
           case_when(
             ind_tod > threshold ~ 1, 
         ...
         ind_deathloc = 
           case_when(
             ind_deathloc > threshold ~ 1, 
             TRUE ~ 0),
         c_ind_fachoused = 
           case_when(
             c_ind_fachoused > threshold ~ 1, 
             TRUE ~ 0),
         c_ind_cod_type = 
           case_when(
             c_ind_cod_type > threshold ~ 1, 
             TRUE ~ 0),
         )
...
dcra_table_variable_bar <- dcra_table_variable_bar_cleaned %>% 
  ...
  summarize(present = n(),
            missing = 54 - present) %>% 
  ungroup()
dcra_table_variable_bar <- dcra_table_variable_bar %>% 
  rename()

#dcra_table_variable_bar_cleaned <- dcra_table_variable_bar_cleaned %>% 
#  pivot_longer(cols = !system, names_to = "variable", values_to = "present") 
#dcra_table_variable_bar_cleaned
  
#joined_agg_bar <- dcra_table_variable_bar_cleaned %>% 
#  right_join(dcra_table_variable_bar, join_by(variable==variable)) 
#joined_agg_bar
```
...
dcra_table_variable_bar %>% 
  ...
  mutate(status = factor(status, levels = c("present", "missing"))) %>% 
  ...
  labs(title = "Available DCRA Required Field Data in Press Releases", 
        caption = "DCRA required field data availability determined by a >50% threshold, meaning across all press releases for a given system, the data is present for at least 50% of those press releases ",
  ...
  scale_fill_manual(values = c("#bcf6bcff", "#b8d8f0ff")) + 
