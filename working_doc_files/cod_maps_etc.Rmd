...
ggplot() +
  ...
  scale_colour_manual(values = c("dodgerblue4", "dodgerblue3", "darkcyan", "white"), breaks = c("High", "Medium", "Low")) +
  guides(color = guide_legend(override.aes = list(size = 3))) +
  labs(title = "Homicides by LEOs are onlt reported in four systems", color = "Homicide by LEO \nreporting level") + 
  theme_void()
...
passing_cutoffs <- passing_cutoffs |> 
  mutate(press_release_yrs = 2024 - first_yr_pr, 
         covid_19_status = ifelse((covid_19 / press_release_yrs) >= covid_19_est, "Yes", "No"),
         drug_alc_status = ifelse((drug_alcohol / press_release_yrs) >= drug_alc_est, "Yes", "No"),
         natural_status = ifelse((natural / press_release_yrs) >= natural_est, "Yes", "No"),
         suicide_status = ifelse((suicide / press_release_yrs) >= suicide_est, "Yes", "No"),
         unknown_status = ifelse((unknown / press_release_yrs) >= unknown_est, "Yes", "No"),
         execution_status = ifelse((execution / press_release_yrs) >= execution_est, "Yes", "No"),
         homicide_status = ifelse((homicide / press_release_yrs) >= homicide_est, "Yes", "No"),
         hom_by_leo_status = ifelse((homicide_by_leo / press_release_yrs) >= hom_by_leo_est, "Yes", "No"),
         uninten_non_drug_inj_status = ifelse((uninten_non_drug_inj / press_release_yrs) >= uninten_non_drug_inj_est, "Yes", "No"),
         pending_status = ifelse((pending / press_release_yrs) >= pending_est, "Yes", "No"))

summary_cod_cutoffs <- passing_cutoffs |> 
  select(covid_19_status, drug_alc_status, natural_status, suicide_status, unknown_status, execution_status, homicide_status, hom_by_leo_status, uninten_non_drug_inj_status, pending_status, c_system_abbr) |> 
  rename_with(~str_remove(., '_status')) |>
  pivot_longer(cols = covid_19:pending, names_to = "cod_category", values_to = "pass_status") |>
  mutate(pass_status = case_when(
    is.na(pass_status) ~ "No Data", 
