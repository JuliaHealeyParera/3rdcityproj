...
```{r}
dcra_variables <- c("c_facility_name", "c_ind_first", "c_ind_last", "c_ind_gender", "c_ind_race", "c_ind_ethnicity", "c_ind_dob_year", "c_ind_dod_ymd", "ind_tod", "ind_deathloc", "combo_COD_c")
system_variable <- c('c_system_abbr')
```

#TESTING - can delete later
```{r}
temp_data <- read_csv('individual_tbl.2024-04-30.csv')
temp <- temp_data %>% 
  select(c_facility_name) %>% 
