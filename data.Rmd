...
agg_data <- read_csv(here('data', 'aggregate_data.csv'))
count(agg_data)
```

#Cause of Death Miscoding Based on Edge Cases 
```{r}
negative_keywords <- c("not", "non", "never", "unknown", "investigate", "undetermined", "conduct") 
data %>% 
...
#DCRA required fields (time of death, and location of death, description of death (narrative and cause of death))
```{r}
dcra_variables <- c("c_ind_fachoused", "c_ind_first", "c_ind_last", "c_ind_gender", "c_ind_race", "c_ind_ethnicity", "c_ind_dob_year", "c_ind_dod_ymd", "ind_tod", "ind_deathloc", "combo_COD_c")
system_variable <- c('c_system_abbr')
```
...
dcra_dataset <- data %>% 
  select(all_of(c(system_variable, dcra_variables)))
dcra_dataset
```

#Missing data percentage based on DCRA required fields
```{r}
#computes the percentage of missing values for each variable across the entire dataset
data %>% 
  summarize(across(everything(), ~mean(is.na(.)) * 100)) %>% #calculates percentage of missing values for each column
  select(all_of(c(system_variable, dcra_variables))) %>% 
  pivot_longer(cols = everything(), names_to = "variable", values_to = "missing_data_percentage") %>% 
  mutate(missing_data_percentage = round(missing_data_percentage, 2))
```

#Missing data percentage for each variable by system
```{r}
#computes the percentage of missing values for each variable for each system 
data %>% 
  group_by(system) %>% 
  summarize(across(everything(), ~mean(is.na(.)) * 100)) %>% 
  pivot_longer(cols = -system, names_to = "variable", values_to = "missing_data_percentage") %>% 
  mutate(missing_data_percentage = round(missing_data_percentage, 2)) %>% 
  pivot_wider(names_from = system, values_from = missing_data_percentage) %>% 
...
comprehensive_percentages <- data %>% 
  ...
  pivot_longer(cols = everything(), names_to = "variable", values_to = "missing_comprehensive_percentage") %>% 
  mutate(missing_comprehensive_percentage = round(missing_comprehensive_percentage, 2)) 
...
comprehensive_systems <- data %>% 
  ...
  pivot_longer(cols = -system, names_to = "variable", values_to = "missing_comprehensive_data") %>% 
  filter(missing_comprehensive_data) %>% 
  group_by(variable) %>% 
